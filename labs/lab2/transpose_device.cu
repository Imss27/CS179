...
void naiveTransposeKernel(const float *input, float *output, int n) {
    ...
    // Output is not
    // In a warp i is increasing and output access will change by n and input access will change by 1
    for (; j < end_j; j++)
...
void shmemTransposeKernel(const float *input, float *output, int n) {
    ...
    /*
    Implementation 1: slower than naive transpose
    Reason: Column read for input, row wise write for output.
            Memory access is not coalesced fot output.
    ...
    for (; j < end_j; j++)
        data[i%64][j%64] = input[i + n * j];

    __syncthreads();

    const int out_i = threadIdx.x + 64 * blockIdx.x;
    int out_j = 4 * threadIdx.y + 64 * blockIdx.y;
    const int out_end_j = out_j + 4;

    for (; out_j < out_end_j; out_j++)
        output[out_j + n * out_i] = data[out_i%64][out_j%64];
    */

    /*
    Implementation 2: faster than naive transpose
    Reason: Column read for input, row wise write for output.
            Memory access is not coalesced fot output.
    */
    __shared__ float data[64][64];
    ...
    for (; out_j < out_end_j; out_j++)
        output[out_i + n * out_j] = data[out_j%64][out_i%64];
}
